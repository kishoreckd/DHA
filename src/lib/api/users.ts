...
export const usersApi = {
  ...
  permissions: () => apiRequest<unknown>("/api/gateway/users/permissions"),
  get: (id: string) => apiRequest<User>(`/api/gateway/users/${id}`),
  update: (id: string, input: { role?: UserRole; is_active?: boolean; permissions?: string[] }) =>
  ...
  invite: (input: { email: string; display_name: string; role: UserRole; permissions?: string[] }) =>
